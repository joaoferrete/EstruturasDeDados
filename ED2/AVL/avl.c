...
void * mallocSafe(size_t size){
    //Função que aloca espaço e encerra o programa em caso de erro de alocação
...
AVL * procuraAVL(AVL * ptr, void * inf, int (*compara)(void*, void*)){
    ...
    if(compara(ptr->info, inf)==0) return ptr;  //Caso a raiz atual seja o elemento buscado se retorna um ponteiro para ela
    else if(compara(ptr->info, inf)>0) return procuraAVL(ptr->right, inf, compara); //Caso o elemento buscado seja maior que o
    //                                                                              elemento atual, se faz a busca pela direita
    return procuraAVL(ptr->left, inf, compara);//Caso contrario, se faz a busca pela direita
}
...
AVL * inserirAVL(AVL * ptr, void * inf, int (*compara)(void*, void*)){
    ...
    if(compara(ptr->info, inf)>0){
        ...
        ptr->right=inserirAVL(ptr->right, inf, compara);    //É feita a chamada recursiva para a inserção do elemento da subarvore direita
        ...
        ptr->left=inserirAVL(ptr->left, inf, compara);  //Chamada recursiva para inserção na subarvore esquerda
...
AVL * removeAVL(AVL * ptr, void * inf, int (*compara)(void*, void*), void (*copia)(void**, void**)){ 
    //Função responsável pela remoção de elementos na árvore
    if(!ptr) return ptr; //Caso a arvore seja vazia, retorna-se ela mesma

    if(!procuraAVL(ptr, inf, compara)) return ptr;   //Caso o elemento a ser deletado nao seja encontrado, nada é feito
    ...
    if(compara(ptr->info, inf)==0){
        ...
        if(!ptr->right){
            ...
            copia(&ptr->info, &aux->info);  //É feita a troca dos elementos de informação entre a variável auxiliar e a raiz
            ptr->right = removeAVL(ptr->right, inf, compara, copia);    //É chamada a função de apagar para a subarvore direita
        ...
        ptr->left=removeAVL(ptr->left, inf, compara, copia);    //É feita a chamada recursiva com a subarvore esquerda
